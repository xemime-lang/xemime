    ...
    private Type check(Resolver resolver, ExprNode exprNode) throws TypeError, SemanticError, FatalError {
        ...
        switch (exprNode.getOperator()) {
            ...
            case MOD:
                ...
                else if (tLhs instanceof IntType) throw new TypeError(exprNode.getLocation(), 141, "剰余演算子の右辺が整数型データではありません。");
                else if (tRhs instanceof IntType) throw new TypeError(exprNode.getLocation(), 142, "剰余演算子の左辺が整数型データではありません。");
                else throw new TypeError(exprNode.getLocation(), 143, "剰余演算子の両辺が整数型データではありません。");
            case AND:
            case OR:
            case XOR:
